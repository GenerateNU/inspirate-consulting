.PHONY: help docker-up docker-down docker-stop docker-restart docker-logs \
        ...
        build-backend build-frontend shell-backend shell-frontend ps \
        rebuild rebuild-backend rebuild-frontend watch \
        generate-api clean-node-modules format-frontend
...
# ------------------------
help:
	@echo "$(BOLD)Inspirate Consulting - Root Commands$(NC)"
	@echo ""
	...
	@echo ""
	@echo "$(BLUE)API & Dependencies:$(NC)"
	@echo "  make generate-api      - Regenerate frontend API clients from OpenAPI spec"
...
format-frontend:
	@echo "$(BOLD)Linting and fixing frontend files...$(NC)"
	@cd frontend && npm run lint -- --fix 2>/dev/null || npm run lint
	@echo "$(GREEN)Frontend formatting/lint complete$(NC)"
...
generate-api:
	@echo "$(BOLD)Generating frontend API client with Orval...$(NC)"
	@cd frontend && npx orval
	@echo "$(GREEN)Frontend API client generated!$(NC)"
...
clean-node-modules:
	...
	@echo ""
	@echo "$(BOLD)Reinstalling frontend dependencies...$(NC)"
